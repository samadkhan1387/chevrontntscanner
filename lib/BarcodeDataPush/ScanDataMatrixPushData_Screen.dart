...
class ScanDataMatrixPushData extends ConsumerStatefulWidget {
  final String selectedOrder;
  const ScanDataMatrixPushData({super.key, required this.selectedOrder});

  @override
  ConsumerState<ScanDataMatrixPushData> createState() => _ScanDataMatrixPushDataScreenState();
}

class _ScanDataMatrixPushDataScreenState extends ConsumerState<ScanDataMatrixPushData> with WidgetsBindingObserver implements ScannerCallback {
  ...
  Future<void> pushDataToServer() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> requestData = parsedBarcodeData.map((barcode) {
        return {
          'orderNumber': widget.selectedOrder,
          'productName': barcode['91'],
          'batchNumber': barcode['10'],
          'productionDate': barcode['11'],
          'serialNo': barcode['21'],
        };
      }).toList();
      ...
      if (response.statusCode == 201) {
        ...
        if (responseBody is List && responseBody.isNotEmpty && responseBody[0]['error'] == 'Serial number must be unique.') {
    ...
    if (data != null) {
      ...
      try {
        ...
        if (barcodeData.containsKey('10') &&
          ...
          await dbHelper.insertPushedBarcode(barcodeData, widget.selectedOrder);
          final savedRecord = await dbHelper.getAllPushedBarcodes();
          print('📦 Pushed Barcode Saved to DB: $savedRecord');
      ...
      bottomNavigationBar: Container(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton.icon(
            label: const Text("Push Data To Server"),
