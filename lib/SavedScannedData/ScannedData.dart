...
class ScannedDataScreen extends ConsumerStatefulWidget {
  const ScannedDataScreen({super.key});

  @override
  ConsumerState<ScannedDataScreen> createState() => _ScannedDataScreenState();
}

class _ScannedDataScreenState extends ConsumerState<ScannedDataScreen> {
  ...
  // Future<void> loadSavedBarcodes() async {
  //   final dbHelper = BarcodeDatabaseHelper();
  //   final allBarcodes = await dbHelper.getAllBarcodes();
  //
  //   setState(() {
  //     parsedBarcodeData = allBarcodes.map<Map<String, String>>((barcode)  {
  //       return {
  ...
  Future<void> loadSavedBarcodes() async {
    ...
    setState(() {
      parsedBarcodeData = allBarcodes.map<Map<String, String>>((barcode) {
        return {
          'sid': barcode['sid'].toString(), // include sid!
          '91': barcode['productName'] ?? '',
          '10': barcode['batchNumber'] ?? '',
          '11': barcode['productionDate'] ?? '',
          '21': barcode['serialNumber'] ?? '',
                            ...
                            children: [
                              ...
                              IconButton(
                                icon: const Icon(Icons.delete,
                                    color: Colors.black),
                                onPressed: () async {
                                  final sid = int.tryParse(barcodeData['sid'] ?? '');
                                  if (sid != null) {
                                    final dbHelper = BarcodeDatabaseHelper();
                                    await dbHelper.deleteById(sid);
